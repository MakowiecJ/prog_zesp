function updateSchedule() {
    ...
    xhr.open('POST', 'http://localhost:8083/movies/repertoire/edit', true)
...
function addEventListeners() {
    ...
    aside.addEventListener('drop', (e) => { 
        ...
        if (cell.id.indexOf('n') == -1) {
            ...
            xhr.open('DELETE', 'http://localhost:8083/movies/screening/' + id, true)
            xhr.setRequestHeader("content-type", "application/json")
...
const getRepertoire = async (city, date) => {
    const repertoire = fetch('http://localhost:8083/movies/repertoire?city=' + city + '&date=' + date)
        .then((response) => response.json())
        .then((data) => {
            return data
        })
